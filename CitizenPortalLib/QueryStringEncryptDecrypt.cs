...
{
    /// <summary>
    /// Summary description for QueryStringEncryptDecrypt
    /// </summary>
    internal class QueryStringEncryptDecrypt
    {
        ...
        public static QryStringsCollection DecryptMultipleQueryString(String strQueryString)
        {
            String str = "";
            string[] split = strQueryString.Split(',');
            for (int i = 0; i < split.Length; i++)
            {
                if (i != 0)
                {
                    str = str + "&";
                    split[i] = split[i].Replace("str=", "");
                }
                byte[] byt = Convert.FromBase64String(split[i].Replace(" ", "+"));
                MemoryStream ms = new MemoryStream();
                DESCryptoServiceProvider des = new DESCryptoServiceProvider();
                CryptoStream cs = new CryptoStream(ms, des.CreateDecryptor(key, IV), CryptoStreamMode.Write);
                cs.Write(byt, 0, byt.Length);
                ...
                str = str + encoding.GetString(ms.ToArray());
