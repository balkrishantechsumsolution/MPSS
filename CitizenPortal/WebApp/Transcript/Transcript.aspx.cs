...
namespace CitizenPortal.WebApp.Transcript
{
    public partial class Transcript : System.Web.UI.Page
    {
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static string GetSemester(string Course, string Program)
        {
            string text;
            FacultyModuleBLL m_FacultyModuleBLL = new FacultyModuleBLL();
            System.Data.DataTable dtAppList = m_FacultyModuleBLL.GetSemesterCSVTU(Course, Program);
            text = JsonConvert.SerializeObject(dtAppList, Newtonsoft.Json.Formatting.Indented);
            return text;
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static string InsertTranscriptData(string prefix, string Data,string SemesterInfo)
        {
            string noNewLines = Data.Replace("\n", "");
            CBACSVTU_DT viewModel = ToObjectFromJSON<CBACSVTU_DT>(noNewLines);
            ...
            viewModel.SemesterInfo = SemesterInfo;
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(binding))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertTranscriptCBSAData(viewModel);
                }
            }
