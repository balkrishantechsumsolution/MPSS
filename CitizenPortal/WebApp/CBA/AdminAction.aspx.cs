...
namespace CitizenPortal.WebApp.CBA
{
    public partial class AdminAction : System.Web.UI.Page
    {
        ...
        FacultyModuleBLL m_FacultyModule = new FacultyModuleBLL();
        string m_ProfileID = "";
        ...
        protected void btnApproved_Click(object sender, EventArgs e)
        {
            ...
            string t_ActionID = ddlEntrance.SelectedValue;
            string t_Remarks = txtRemark.Text.Trim();
            string t_CreatedBy = Session["LoginID"].ToString();
            string t_IPAddress = Request.UserHostAddress;

            if (ddlEntrance.SelectedValue == "0") {
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Message", "alert('Please select Action Type');", true);
                return;
            }

            t_Result = m_WorkFlowBLL.CBAAdminAction(m_ServiceID, m_AppID, t_ActionID, t_Remarks, Session["LoginID"].ToString(), t_IPAddress);

            if (t_Result != null && t_Result.Rows.Count > 0)
            {
                CitizenPortalLib.EMailSMS test = new CitizenPortalLib.EMailSMS();

                string MobileNo = t_Result.Rows[0]["MobileNo"].ToString();
                string SMSText = t_Result.Rows[0]["SMSText"].ToString();

                string MailID = t_Result.Rows[0]["EmailID"].ToString();
                string subject = t_Result.Rows[0]["Subject"].ToString();
                string Mailtext = t_Result.Rows[0]["MailText"].ToString();
                string bcc = t_Result.Rows[0]["BCCMailID"].ToString();

                if (Mailtext != "")
                {
                    CommonUtility.SendEmailWithAttachment("", "", "", MailID, null, bcc, subject, Mailtext, true, null);
                }
                if (MobileNo != "")
                ...
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Message",
                     "alert('Unable to Update Status.');", true);
            }
        }
