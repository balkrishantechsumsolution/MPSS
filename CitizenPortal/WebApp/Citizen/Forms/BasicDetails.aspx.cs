...
namespace CitizenPortal.WebApp.Citizen.Forms
{
    public partial class BasicDetails : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string InsertCitizenBasicDetails(string prefix, string Data)
        {
            string noNewLines = Data.Replace("\n", "");
            CitizenBasicDetails_DT viewModel = ToObjectFromJSON<CitizenBasicDetails_DT>(noNewLines);
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(new BasicHttpBinding()))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertCitizenBasicDetails(viewModel);
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        [WebMethod]
        public static string GetUserDetails(string prefix, string UID)
        {

            string noNewLines = UID.Replace("\n", "");
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(new BasicHttpBinding()))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.GetUserDetails(prefix, UID);
