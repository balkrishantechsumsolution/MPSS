...
namespace CitizenPortal.WebApp.Kiosk.DOF
{
    public partial class RegistrationForm : BasePage
    {
        ...
        [WebMethod]
        public static string InsertFisheriesRegistration(string prefix, string Data)
        {
            string noNewLines = Data.Replace("\n", "");
            FisheriesRegistration_DT viewModel = ToObjectFromJSON<FisheriesRegistration_DT>(noNewLines);
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(new BasicHttpBinding()))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertFisheriesRegistration(viewModel);
                }
            }
            return text;
        }
