...
namespace CitizenPortal.WebApp.Kiosk.Death
{
    public partial class DeathForm : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string InsertDeathCertificate(string prefix, string Data)
        {
            string noNewLines = Data.Replace("\n", "");
            DeathCertificate_DT viewModel = ToObjectFromJSON<DeathCertificate_DT>(noNewLines);
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(new BasicHttpBinding()))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertDeathCertificate(viewModel);
