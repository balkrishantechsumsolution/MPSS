...
{
    public partial class IGNOAPForm : BasePage
    {
        string UID = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            HiddenIGNOP.Value = dt.Rows[0]["dateOfBirth"].ToString();
            HiddenIGNOPGender.Value = dt.Rows[0]["gender"].ToString();

            Session["CurrentCulture"] = culture;
            HFServiceID.Value = Request.QueryString["SvcID"].ToString();
            ngServiceID.Value = Request.QueryString["SvcID"].ToString();
        ...
        [WebMethod]
        public static string InsertIGNOAP(string prefix, string Data)
        {
            string noNewLines = Data.Replace("\n", "");
            IGNOAP_DT viewModel = ToObjectFromJSON<IGNOAP_DT>(noNewLines);
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(new BasicHttpBinding()))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertIGNOAP(viewModel);
