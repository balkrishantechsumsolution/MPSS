...
{
    public partial class IGNWPForm : BasePage
    {
        string UID = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            HiddenIGNWPAge.Value = dt.Rows[0]["dateOfBirth"].ToString();
            HiddenIGNWPGender.Value = dt.Rows[0]["gender"].ToString();

            Session["CurrentCulture"] = culture;
            HFServiceID.Value = "107";
            ngServiceID.Value = "107";
        ...
        [WebMethod]
        public static string InsertIGNWP(string prefix, string Data)
        {
            string noNewLines = Data.Replace("\n", "");
            IGNWP_DT viewModel = ToObjectFromJSON<IGNWP_DT>(noNewLines);
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(new BasicHttpBinding()))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertIGNWP(viewModel);
