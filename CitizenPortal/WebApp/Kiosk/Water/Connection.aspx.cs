...
namespace CitizenPortal.WebApp.Kiosk.Water
{
    public partial class Connection : BasePage
    {
        ...
        [WebMethod]
        public static string InsertWaterConnection(string prefix, string Data)
        {
            string noNewLines = Data.Replace("\n", "");
            WaterConnection_DT viewModel = ToObjectFromJSON<WaterConnection_DT>(noNewLines);
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(new BasicHttpBinding()))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertWaterConnection(viewModel);
