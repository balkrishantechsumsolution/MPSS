...
{
    public partial class Scrutiny : BasePage
    {
        ...
        private void BindGrid()
        {
            ...
            DataSet dt = m_OISFBLL.OSIFScrutinyData(m_ServiceID, LoginID, DistrictCode, category, FromDate, ToDate);
            ...
            for (int i = 0; i < pnlGrid.Controls.Count; i++)
            {
                ...
                if (t_Action != "")
                {
                    //if (RowID != oldRowID)
                    {
                        ...
                        try
                        {
                            t_Result = m_OISFBLL.ScrutinyApplication("388", RowID, t_Action, t_Reason, "", LoginID);//TODO: TO be activated after proper verififcation.
