...
namespace CitizenPortal.WebApp.Kiosk.OISF
{
    public partial class AdmitCard : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                string m_Message = "Please enter Applicant Date of Birth, Registered Mobile Number to download Admit Card.";
            ...
            if (txtRollNO.Text == "" && txtLogin.Text == "" && txtBirthDate.Text == "" && txtAppID.Text == "" && txtMobile.Text == "") {
                string m_Message = "Please enter at least three fields to Download Admit Card";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + m_Message + "');", true);
                return;
            }
            ...
            if (captcha.Text == "")
            {
                string m_Message = "Please enter captcha to download Admit Card";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + m_Message + "');", true);
                return;
            }
            ...
            {
                string m_Message = "Invalid Captcha. Please Refresh the Page to download Admit Card";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + m_Message + "');", true);
                return;
            }
            ...
            try
            {
                ...
                if (t_Result != null && t_Result.Rows.Count > 0)
                {
                    if (t_Result.Rows[0]["Result"].ToString() != "")
                    {
                        ...
                        Response.Redirect("/WebApp/Kiosk/OISF/OISFAdmitCard.aspx?SvcID=" + m_ServiceID + "&AppID=" + m_AppID);
                    }
                    else
                    {                        
                        string m_Message = "Sorry! Your Admit Card not announced, as you have not qualified in Physical Efficiency Examination.";
                ...
                {
                    string m_Message = "No record found againt the data entered. Please enter correct information to Download Admit Card.";
                    divErr.InnerHtml = m_Message;
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + m_Message + "');", true);
                }
            }
