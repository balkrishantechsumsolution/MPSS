function SubmitData() {
    ...
    var datavar = {
        ...
        'ExamName': $('#ddlExam option:selected').val(),                 
        'Reason': $('#txReason').val(),
    ...
    $.when(
        $.ajax({
            ...
            url: '/WebApp/Kiosk/ProvisionalSU/Provisional.aspx/InsertProvisional',
...
function ValidateForm() {
    ...
    var ddlpassed = $("#ddlPassing").val();
    var College = $("#ddlCollege").val();

    var ddlResult = $("#ddlResult").val();
    ...
    if (ddlResult == null || ddlResult == " " || ddlResult == "0") {
        text += "<br /> -Please select Examination result";
        $('#ddlResult').attr('style', 'border:1px solid #d03100 !important;');
        $('#ddlResult').css({ "background-color": "#fff2ee" });
        opt = 1;
    } else if (ddlResult != null) {
        $('#ddlResult').attr('style', 'border:1px solid #cdcdcd !important;');
        $('#ddlResult').css({ "background-color": "#ffffff" });
    }

    if (ddlExam == null || ddlExam == " " || ddlExam == "0") {
        text += "<br /> -Please select Examination Type";
        $('#ddlExam').attr('style', 'border:1px solid #d03100 !important;');
        $('#ddlExam').css({ "background-color": "#fff2ee" });
        opt = 1;
    } else if (ddlExam != null) {
        $('#ddlExam').attr('style', 'border:1px solid #cdcdcd !important;');
        $('#ddlExam').css({ "background-color": "#ffffff" });
    }
