...
$(document).ready(function () {
    debugger;
    if ($("#hdfAffiliationNo").val() == "0") {
        var a = confirm('You need to upload College Affiliation Details with Chhattisgarh Swami Vivekanad Technical University to continue +3 CBCS Enrollment for 2020-21');
        if (a == true) {
            window.location.href = "/WebApp/Kiosk/CBCS/CollegeProfile.aspx";
        }
        else {
            alert('You are not entitled to continue +3 CBCS Enrollment for 2020-21');
            window.location.href = "/WebApp/G2G/SU/SUDashboardNew.aspx";
        }
    }
    //else { window.location.href = "/WebApp/G2G/SU/SUDashboardNew.aspx"; }
...
function CheckCollegeAffiliation(){
    debugger;
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/WebApp/Kiosk/CBCS/DEOAdmissionForm.aspx/CollegeAffiliation',
        data: '{}',
        dataType: "json",
        success: function (response) {
            var Category = eval(response.d);
