...
function ValidateRegisteredMobile(Mobile, Type)//Here Type use for validation 1-Profile,2-OUATFormA
{
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/WebApp/Kiosk/OUAT/UG/FormA.aspx/ValidateRegisteredMobile',
        ...
        success: function (response) {
            ...
            if (Result == '0') {
                ...
                var rtnurl = "/WebApp/Kiosk/OUAT/OUATPage.aspx";
                window.location.href = rtnurl;
            }
            else {
                $("#txtMobile").prop('disabled', false);
...
function fnGenOTP(ProfileStatus) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/WebApp/KIOSK/OUAT/UG/FormA.aspx/GenerateOUATOTP',
        ...
        else if (Status == 'AlreadyExist')
        {
            ...
            alertPopup("Mobile No already exist!", "Mobile No already registered for Admission into OUAT Master's & Doctoral Program-2019-20 (Agriculture & Allied Subjects), Please login to check application status.");
        }
        else {
...
function fnValidateOUATOTP() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/WebApp/KIOSK/OUAT/UG/FormA.aspx/ValidateOTPOUAT',
...
function fnOTPValidaed(UID, KeyField, ResponseType, ProfileID, AadhaarKeyField) {
    ...
    if (ResponseType == "M") {
        ...
        $("#divMsgTitle").text("Information!");
        $("#divMsgDtls").text("Mobile no " + $('#txtMobile').val() + " sucessfully validated. Please Fill Application Form for Admission into M. Sc. And PhD– 2019 (College of Basic Science and Humanities)");
        $('#divMsg').hide();

        alert('Mobile No. ' + $('#txtMobile').val() + ' sucessfully validated. Please Fill Application Form for Admission into M. Sc. And PhD– 2019 (College of Basic Science and Humanities)');
        ...
        if (KeyField == '0000000000000') {
            ...
            $('#btnSubmit').prop('disabled', false);
            $("#divMsgTitle").text("Information!");
            $("#divMsgDtls").text("Mobile no " + $('#txtMobile').val() + " sucessfully validated. Please Fill Application form for Admission into OUAT Master's & Doctoral Program-2019-20 (Agriculture & Allied Subjects)");
            $('#divMsg').show();

            alert("Mobile No. " + $('#txtMobile').val() + " sucessfully validated. Please Fill Application form for Admission into OUAT Master's & Doctoral Program-2019-20 (Agriculture & Allied Subjects)");

        } else {
            alert("Mobile No. " + $('#txtMobile').val() + " already registered for Application form for Admission into OUAT Master's & Doctoral Program-2019-20 (Agriculture & Allied Subjects). Please Login to check the details.");
            rtnurl = "/WebApp/Kiosk/OUAT/OUATPage.aspx";
            window.location.href = rtnurl;
...
function ValidateForm() {
    ...
    */
    ...
    if (parentincome == '' || parentincome == 'Select' || parentincome == '0') {
        ...
        $('#ddlParentAnnumIncome').attr('style', 'border:1px solid #d03100 !important;');
        $('#ddlParentAnnumIncome').css({ "background-color": "#fff2ee" });
        opt = 1;
    }
...
function SubmitData() {
    ...
    $.when(
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: '/WebApp/Kiosk/OUAT/PGPhD/PGPhdApplication.aspx/InsertData',
