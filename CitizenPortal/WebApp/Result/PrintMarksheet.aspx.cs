...
{
    public partial class PrintMarksheet : AdminBasePage
    {
        ...
        public void PrintGrid()
        {
            ...
            string m_PrintFlag = "N";
            //ds = obj.GetAttendentSheetDetails(m_coursecode, m_CollegeCode, m_Semester, m_ExamType, m_ExamYear, m_Range, m_RollNo, nextset);
        ...
        protected void grdAttendanceSheet_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                img.Src = dtGlobal.Rows[e.Row.RowIndex]["photograph"].ToString();
        ...
        {
            DataSet ds = new DataSet();
            string m_coursecode = "", m_BranchCode = "", m_CollegeCode = "", m_ExamType = "", m_ExamYear = "", m_Semester = "", m_Range = "", m_RollNo = "", nextset = "";
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                dtGlobal = ds.Tables[0];
                grdAttendanceSheet.DataBind();
                //string enrollme = ds.Tables[0].Rows[0]["EnrollmentNo"].ToString();
                //string rollno = ds.Tables[0].Rows[0]["RollNo"].ToString();
                //string namecand = ds.Tables[0].Rows[0]["name"].ToString();
                //string semes = ds.Tables[0].Rows[0]["name"].ToString();
                //string fath = ds.Tables[0].Rows[0]["name"].ToString();
                //string colgname = ds.Tables[0].Rows[0]["name"].ToString();
                //string rslt = ds.Tables[0].Rows[0]["name"].ToString();
                //string noexm = ds.Tables[0].Rows[0]["programname"].ToString();
                //string date = ds.Tables[0].Rows[0]["ResultDate"].ToString();
                //QRCodeGenerator qrGenerator = new QRCodeGenerator();
                //QRCodeData qrCodeData = qrGenerator.CreateQrCode("\nEnrollment NO:" + enrollme + " \nRoll No.:" + rollno + " \nName:" + namecand + "  \nFather's Name: " + fath + "  \nInstitute Name: " + colgname + " \nExam Name: " + noexm + " \nSemester:" + semes + " \nResult Status:" + rslt + " \nDate:" + date, QRCodeGenerator.ECCLevel.Q);
                //QRCode qrCode = new QRCode(qrCodeData);
                //System.Web.UI.WebControls.Image imgBarCode = new System.Web.UI.WebControls.Image();
                //imgBarCode.Height = 108;
                //imgBarCode.Width = 119;
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataSet ds = new DataSet();
                string m_coursecode = "", m_CollegeCode = "", m_ExamType = "", m_ExamYear = "", m_Semester = "", m_Range = "", m_RollNo = "", nextset = "";
        ...
        protected void grdAttendanceSheet_ItemDataBound(object sender, DataListItemEventArgs e)
        {
            ...
            t_DS = obj.GetResultData(dtGlobal.Rows[e.Item.ItemIndex]["RollNo"].ToString(), ddlSemester.SelectedItem.Value, ddlExamType.SelectedItem.Value, ddlSession.SelectedItem.Value);
             ...
             e.Item.ItemType == ListItemType.AlternatingItem)
            {
                if (e.Item.DataItem != null)
                {
                    ...
                    if (t_DS1.Tables[0].Rows.Count > 0)
                    {
                        ...
                        Label Srno = (e.Item.FindControl("Sno") as Label);
                        ...
                        Srno.Text = t_DS1.Tables[0].Rows[0]["SerialNo"].ToString();
                    ...
                    if (t_DS2.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (ddlSemester.SelectedValue.ToString().ToUpper() == "8 SEMESTER")
                        {
                    ...
                    try
                    {
                        //QRCode1.GenerateQRCodeMarksheet(dtGlobal.Rows[e.Item.ItemIndex]["RollNo"].ToString(), ddlSemester.SelectedItem.Value, ddlExamType.SelectedValue.ToString(), ddlSession.SelectedItem.Value);
                        QRCode QRCodeMS = (e.Item.FindControl("QRCode1") as QRCode);
                        //QRCodeMS.GenerateQRCodeMarksheet(dtGlobal.Rows[e.Item.ItemIndex]["RollNo"].ToString(), ddlSemester.SelectedItem.Value, ddlExamType.SelectedValue.ToString(), ddlSession.SelectedItem.Value);
                        string SubjectDetails = "";
                        DataTable MarkDetails = null;
                        if (t_DS.Tables[0].Rows.Count > 0)
                        {
                            MarkDetails = t_DS.Tables[0];
                            for (int i = 0; i < MarkDetails.Rows.Count; i++)
                            {
                                if (i == 0)
                                {
                                    SubjectDetails = MarkDetails.Rows[i]["Subject_Code"].ToString() + ":" + MarkDetails.Rows[i]["totalobt"].ToString();
                                }
                                else
                                    SubjectDetails = SubjectDetails + "\n" + MarkDetails.Rows[i]["Subject_Code"].ToString() + ":" + MarkDetails.Rows[i]["totalobt"].ToString();
                            }

                        }
                        QRCodeMS.GenerateQRCode(
                            "Enrollment NO:" + t_DS3.Tables[0].Rows[0]["EnrollmentNo"].ToString() +
                            "\nRoll No.:" + t_DS3.Tables[0].Rows[0]["RollNo"].ToString() +
                            "\nName:" + t_DS3.Tables[0].Rows[0]["Name"].ToString() +
                            "\nFather's Name:" + t_DS3.Tables[0].Rows[0]["Father"].ToString() +
                            "\nInstitute Name:" + t_DS3.Tables[0].Rows[0]["CollegeName"].ToString().Replace("&", "and") +
                            "\nExam Name:" + t_DS3.Tables[0].Rows[0]["ProgramName"].ToString() +
                            "\nSemester:" + t_DS3.Tables[0].Rows[0]["Semester"].ToString() +
                            "\n" + SubjectDetails +
                            "\nMarks Obtained:" + t_DS3.Tables[0].Rows[0]["totalobt"].ToString() +
                            "\nTotal Marks:" + t_DS3.Tables[0].Rows[0]["total"].ToString() +
                            "\nResult:" + t_DS3.Tables[0].Rows[0]["lettergrade"].ToString() +
                            "\nDate:" + t_DS3.Tables[0].Rows[0]["ResultDate"].ToString());
                    }
                    catch
                    {
                    }
        ...
        //public void qrgenrt()
        //{
        //    Label enrollme = (e.Item.FindControl("EnrollmentNo") as Label).Text;
        //    Label rollno = (e.Item.FindControl("lblRollNo") as Label).Text;
        //    Label namecand = (e.Item.FindControl("lblname") as Label).Text;
        //    Label semes = (e.Item.FindControl("lblSemester") as Label).Text;
        //    Label fath = (e.Item.FindControl("lblFather") as Label).Text;
        //    Label colgname = (e.Item.FindControl("lblNOI") as Label).Text;
        //    Label rslt = (e.Item.FindControl("lblResult") as Label).Text;
        //    Label noexm = (e.Item.FindControl("lblprogramname") as Label).Text;
        //    Label totmks = (e.Item.FindControl("total") as Label).Text;
        //    Label mob = (e.Item.FindControl("totalobt") as Label).Text;
        //    Label date = (e.Item.FindControl("DateResult") as Label).Text;
        //    QRCodeGenerator qrGenerator = new QRCodeGenerator();
        //    QRCodeData qrCodeData = qrGenerator.CreateQrCode("\nEnrollment NO:" + enrollme + " \nRoll No.:" + rollno + " \nName:" + namecand + "  \nFather's Name: " + fath + "  \nInstitute Name: " + colgname + " \nExam Name: " + noexm + " \nSemester:" + semes + " \nMarks Obtained:" + mob + " \nTotal Marks:" + totmks + " \nResult Status:" + rslt + " \nDate:" + date.Text, QRCodeGenerator.ECCLevel.Q);
        //    QRCode qrCode = new QRCode(qrCodeData);
        //    System.Web.UI.WebControls.Image imgBarCode = new System.Web.UI.WebControls.Image();
        //    imgBarCode.Height = 108;
        //    imgBarCode.Width = 119;
