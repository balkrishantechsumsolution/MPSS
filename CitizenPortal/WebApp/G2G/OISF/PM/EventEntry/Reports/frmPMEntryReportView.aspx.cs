...
{
    public partial class frmPMEntryReportView : System.Web.UI.Page
    {
...

        {
            ...
            DataSet ds = obj.GetOISFAppDetails("OP19", drpCenter.SelectedValue.ToString(), drpday.SelectedValue.ToString(), drpEventType.SelectedValue, "", "", drpStatus.SelectedValue, "", "", "", "");

            if (ds.Tables.Count>0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    grdPMReport.DataSource = ds.Tables[0];
                }
                else
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('No Data Found!!!')", true);
                    DataTable dt = null;
                    grdPMReport.DataSource = dt;
                }
            }
            else
            {
        ...
        {
            GridView grdReport = grdPMReport;
