...
using static CitizenPortal.g2c.forms.OISF;
...
{
    public partial class SCG2GAction : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //disable checkbox when login user not a nodal officer
            if (!LoginID.Contains("PSNodalOfficer") && !LoginID.Contains("NO.") && !LoginID.Contains("ANO."))
            {
                //Page.ClientScript.RegisterStartupScript(this.GetType(), "myScript", "DisableCheckbox();", true);
                ClientScript.RegisterStartupScript(this.GetType(), "print", "<script>DisableCheckbox();</script>");
            ...
            if (t_DT != null && t_DT.Rows.Count > 0)
            {
                ...
                string DeptUrl = "&Dept=Y";
                HFAckURL.Value = t_DT.Rows[0]["FullURL"].ToString() + DeptUrl.ToString();
            }
        ...
        void DisplayAppDetail()
        {
            ...
            //getting document falg details /CK.Photograph,CK.PresentAddress,CK.OriginalAadhar,CK.Mobile,CK.AnyCriminalCase/
            if (dt.Rows.Count > 0)
            {
                if (Convert.ToString(dt.Rows[0]["Photograph"]) != "")
                {
                    string chk1 = dt.Rows[0]["Photograph"].ToString().ToLower() + "," + dt.Rows[0]["PresentAddress"].ToString().ToLower() + ","
                    + dt.Rows[0]["OriginalAadhar"].ToString().ToLower() + "," + dt.Rows[0]["Mobile"].ToString().ToLower() + "," + dt.Rows[0]["AnyCriminalCase"].ToString().ToLower();
                    HdfCheckBox.Value = chk1;
                }
        ...
        protected void LinkButton_Click(object sender, EventArgs e)
        {
            ...
            if (m_DisplayFileUpload)
            {
                if (m_ServiceID == "392" || m_ServiceID == "393")
                {
                    result = UploadReport("FU_DOC998,FU_DOC997,FU_DOC996");
            ...
            if (t_Result != null && t_Result.Rows.Count > 0)
            {
                ...
                dtsms = m_WorkFlowBLL.GetSeniorCitizenSMS(m_ServiceID, m_AppID);
                ...
                if(dtsms != null && dtsms.Rows.Count >0)
                {
                    ...
                    //send to applicant
                    if (dtsms.Rows[0]["ApplicantNo"].ToString() != "")
                    {
                       
                        SMSServices.sendsms(dtsms.Rows[0]["ApplicantNo"].ToString(), dtsms.Rows[0]["ApplicantSMS1"].ToString());
                    }
                    //send to nodal officer
                    if (dtsms.Rows[0]["NodalOfficerNo"].ToString() != "")
                    {
                        SMSServices.sendsms(dtsms.Rows[0]["NodalOfficerNo"].ToString(), dtsms.Rows[0]["NodalSMS2"].ToString());
                    }
                    //send to IIC Officer
                    if (dtsms.Rows[0]["IICOfficerNo"].ToString() != "")
                    {
                        SMSServices.sendsms(dtsms.Rows[0]["IICOfficerNo"].ToString(), dtsms.Rows[0]["IICSMS3"].ToString());
                    }
                    //send to DCP Crime
                    if (dtsms.Rows[0]["DCPCrimeNo"].ToString() != "")
                    {
                        SMSServices.sendsms(dtsms.Rows[0]["DCPCrimeNo"].ToString(), dtsms.Rows[0]["DCPSMS4"].ToString());
                    }
                    //send to ACP 
                    if (dtsms.Rows[0]["ACPReservedNo"].ToString() != "")
                    {
                        SMSServices.sendsms(dtsms.Rows[0]["ACPReservedNo"].ToString(), dtsms.Rows[0]["ACPSMS5"].ToString());
                    }
                ...
                {
                    //Generate ID Card no for Senior Citizen After App Approved. GenerateSeniorCitizenIDCardNoSP
                    DataTable dt = new DataTable();
                    dt = m_WorkFlowBLL.GenerateSeniorCitizenIDCardNo(m_AppID,"1");
                    if (Convert.ToString(dt.Rows[0]["Result"]) == "1")
                    {
                        ...
                        if (MobileNo != "")
                            test.sendsms(MobileNo, dt.Rows[0]["ApplicantSMS"].ToString());
                    }
                }
                ...
                {
                    DataTable dt = new DataTable();
                    dt = m_WorkFlowBLL.GenerateSeniorCitizenIDCardNo(m_AppID, "2");
                    CitizenPortalLib.EMailSMS test = new CitizenPortalLib.EMailSMS();
                    if (Convert.ToString(dt.Rows[0]["Result"]) == "1")
                    {
                        string MobileNo = dt.Rows[0]["MobileNo"].ToString();
        ...
        [WebMethod]
        public static string InsertSeniorCitizenCheckList(string prefix, string Data)
        {
            string noNewLines = Data.Replace("\n", "");
            SeniorCitizenCheckList_DT viewModel = ToObjectFromJSON<SeniorCitizenCheckList_DT>(noNewLines);
            ...
            //{
            //    viewModel.AppID = HttpContext.Current.Session["AppID"].ToString();
            //}
            ...
            using (ChannelFactory<IService1Channel> factory = new ChannelFactory<IService1Channel>(binding))
            {
                ...
                using (OperationContextScope scope = new OperationContextScope(proxy))
                {
                    ...
                    text = proxy.InsertSeniorCitizenCheckList(viewModel);
                }
            }
