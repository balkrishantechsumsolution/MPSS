...
namespace CitizenPortal.WebApp.Activity
{
    public partial class ActivityConfiguration : AdminBasePage
    {
        ...
        private void GetSemester()
        {
            ...
            dt = m_FacultyModuleBLL.GetSemesterCSVTU(ddlCourse.SelectedValue, "");
        ...
        public void LoadGridData(string p_ActionType)
        {
            ...
            DataSet ds = null;
            ds = m_G2GDashboardBLL.GetActivityCSVTU(LoginID, Semester, Activity, t_Year, hdfActionType.Value, ddlCourse.SelectedValue);
        ...
        {            
            ...
            || e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    m_DropDownList.ID = "ddlCourse_" + e.Row.Cells[1].Text;
                    ...
                    DT = m_FacultyModuleBLL.GetCourseCSVTU("");
                        //m_G2GDashboardBLL.GetEditSubjectList(SubType, Semester, BranchCode);

                    if (DT.Rows.Count != 0)
                    {
                        m_DropDownList.Items.Clear();
                        m_DropDownList.DataTextField = "CourseName";
                        m_DropDownList.DataValueField = "CourseCode";
                        ...
                        m_DropDownList.CssClass = "form-control";
                        m_DropDownList.Width = 120;
                        //m_DropDownList.Items.FindByText(SubName).Selected = true;
                        m_DropDownList.SelectedIndex = m_DropDownList.Items.IndexOf(m_DropDownList.Items.FindByValue(ddlCourse.SelectedValue));
                        e.Row.Cells[2].Controls.Add(m_DropDownList);
                    }
                }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string t_Semester = "";
            string t_Year = "";
            string t_Course = ddlCourse.SelectedValue;
            ...
            try
            {
                ...
                for (int i = 0; i < t_RowIDArr.Length; i++)
                {
                    foreach (GridViewRow rows in DataGridView.Rows)
                    {
                        ...
                        System.Web.UI.Control ddlYear = row.FindControl("ExamYear");
                        //System.Web.UI.Control ddlCourse = row.FindControl("ddlCourse_");
                        ...
                        if (chkenroll.Checked)
                        {
                            ...
                            if(chk_RowID == t_RowIDArr[i])
                            {
                                ...
                                if (hdfActionType.Value == "2" || hdfActionType.Value == "1")
                                {
                                    ...
                                    //DropDownList ddlA = (DropDownList)ddlCourse;
                                    //t_Course = ddlA.SelectedItem.Value;
                                }
                                                                
                                Label t_txtCreatedOn = (Label)rows.FindControl("CreatedOn");                                

                                TextBox t_txtStartDate = (TextBox)rows.FindControl("txtStartDate");
                                TextBox t_txtEndDate = (TextBox)rows.FindControl("txtSEMEndDate");

                                TextBox t_txtResultDate = (TextBox)rows.FindControl("txtResultDate");
                                TextBox t_txtMarkEntryEndDate = (TextBox)rows.FindControl("txtMarkEntryEndDate");
                                TextBox t_txtAttendanceEndDate = (TextBox)rows.FindControl("txtAttendanceEndDate");
                                ...
                                {
                                    t_Script = "Please select Course of the Exam!";
                                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Course", "alert('" + t_Script + "');", true);
                                    return;
                                }
                                ...
                                if (t_txtAttendanceEndDate.Text == "") { t_Script = "Attendance End Date is blank!"; ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Activity Start Date", "alert('" + t_Script + "');", true); return; }
                                if (t_txtElectiveSubjectEndDate.Text == "") { t_Script = "Elective End Date is blank!"; ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Activity End Date", "alert('" + t_Script + "');", true); return; }

                                if (t_txtMarkEntryEndDate.Text == "") { t_Script = "Mark Entry End Date is blank!"; ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Activity Start Date", "alert('" + t_Script + "');", true); return; }
                                ...
                                DateTime ResultDate = Convert.ToDateTime(t_txtResultDate.Text);
                                DateTime MarkEntryEndDate = Convert.ToDateTime(t_txtMarkEntryEndDate.Text);
                                ...
                                if (StartDate.CompareTo(AttendanceEndDate) > 0)
                                {
                                    t_Script = "Attendance End Date should be greated then Start Date!";
                                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Date", "alert('" + t_Script + "');", true);
                                    return;
                                }

                                if (StartDate.CompareTo(ElectiveSubjectEndDate) > 0)
                                {
                                    t_Script = "Elective End Date should be greated then Start Date!";
                                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Date", "alert('" + t_Script + "');", true);
                                    return;
                                }

                                if (StartDate.CompareTo(MarkEntryEndDate) > 0)
                                {
                                    t_Script = "Mark Entry End Date should be greated then Start Date!";
                                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Date", "alert('" + t_Script + "');", true);
                                    return;
                                }

                                if (StartDate.CompareTo(OfflineEndDate) > 0)
                                {
                                    t_Script = "Offline End Date should be greated then Start Date!";
                                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Date", "alert('" + t_Script + "');", true);
                                    return;
                                }
                                ...
                                {
                                    t_Script = "Result Declaration Date should be greated then Start Date!";
                                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Invalid Date", "alert('" + t_Script + "');", true);
                                    return;
                                }
                                ...
                                ActivityCSVTU_DT t_Activity_DT = new ActivityCSVTU_DT();

                                t_Activity_DT.RowID = RowID.Value;
                                t_Activity_DT.CourseCode = t_Course;
                                t_Activity_DT.CreatedBy = Session["LoginID"].ToString();
                                t_Activity_DT.Semester = t_Semester;
                                t_Activity_DT.ExamSession = t_Year;
                                ...
                                t_Activity_DT.ResultDate = Convert.ToDateTime(t_txtResultDate.Text).ToString("yyyy-MM-dd HH:mm:ss");
                                t_Activity_DT.MarkEntryEndDate = Convert.ToDateTime(t_txtMarkEntryEndDate.Text).ToString("yyyy-MM-dd HH:mm:ss");
                                ...
                                result = m_AdmissionFormBLL.InsertActivityCSVTU(t_Activity_DT, AFields);
        ...
        {
            if (ddlCourse.SelectedValue == "0" || ddlSemester.SelectedValue == "0" || ddlSession.SelectedValue == "0")
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Please select Course, Semester & Exam Year to edit record!!!')", true);
                DataGridView.DataSource = null;
                divDetails.Visible = false;
                divBtn.Visible = false;
                return;
