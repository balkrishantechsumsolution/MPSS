...
{
    public partial class BulkApproval : System.Web.UI.Page
    {
        ...
        public void BindData()
        {
            ...
            if (ddlServices.SelectedIndex == 0)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alertPopup('Mandatory Selection','Please select Application Type!');", true);
                return;
            }
            if (ddlServices.SelectedIndex != 0)
            {
            ...
            if (ddlServices.SelectedValue == "1499")
            {
            ...
            try
            {
                ...
                t_DT = m_AdmissionFormBLL.GetNominateFacultyData(LoginID, ddlCollege.SelectedValue, ddlDepartment.SelectedValue, ddlProgram.SelectedValue,ddlSubProgram.SelectedValue, ddlCourse.SelectedValue,
                    ddlSession.SelectedValue, ddlSemester.SelectedValue, ddlSubject.SelectedValue, ddlBranch.SelectedValue, Service, FromDate, ToDate, Status, txtAppID.Text.Trim());
        ...
        protected void gvDetail_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (ddlStatus.SelectedValue == "P")
                {
                    if (m_ServiceID == "1450")
                    {
                        t_Anchor.Attributes.Add("onclick", "ViewAdmitCard('" + e.Row.Cells[11].Text + "', '" + ddlServices.SelectedItem.Value.Substring(0, 4) + "', '" + e.Row.Cells[2].Text + "')");
                    }
                    else
                    {
                        t_Anchor.Attributes.Add("onclick", "TakeAction('', '" + ddlServices.SelectedItem.Value.Substring(0, 4) + "', '" + e.Row.Cells[15].Text + "')");
                    }
                }
                else
                {
                    t_Anchor.Attributes.Add("onclick", "TakeAction('', '" + ddlServices.SelectedItem.Value.Substring(0, 4) + "', '" + e.Row.Cells[14].Text + "')");
                }
                t_Anchor.Attributes.Add("style", "cursor:pointer;font-size:10pt;font-family:Arial;color:Firebrick;text-decoration:none;");
        ...
        protected void btn_Submit_Click(object sender, EventArgs e)
        {
            ...
            */
                ...
                foreach (GridViewRow rows in gvDetail.Rows)
                {
                    ...
                    if (chkenroll != null)
                    {
                        ...
                        if (chkenroll.Checked)//if (chk_RowID == t_RowIDArr[i])
                        {
                            ...
                            result = m_AdmissionFormBLL.InsertNominatedFaculty(objNominateFaculty_TB, AFields);
            ...
            if (t_ShowMsg == true)
            {
                ddlStatus.Items.IndexOf(ddlStatus.Items.FindByValue("A"));
                BindData();
                t_Script = "alert('\"{0}\" {1}');";
        ...
        public void GetSubject()
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = m_AdmissionFormBLL.GetTeachersSubject("", ddlSubProgram.SelectedItem.Text, ddlCourse.SelectedItem.Text, ddlSemester.SelectedValue, ddlSession.SelectedValue);
                if (dt != null && dt.Rows.Count > 0)
                {
